...
def run_rope(
    ...
    rope = RoPE(theta=theta, d_k=d_k, context_length=max_seq_len)
...
def run_get_batch(
    ...
    """
    from training import create_training_batch
    return create_training_batch(dataset, batch_size, context_length, device)
...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    """
    from optimizers import clip_gradients
...
def run_get_lr_cosine_schedule(
    ...
    """
    from optimizers import get_cosine_lr_schedule
    return get_cosine_lr_schedule(it, (min_learning_rate, max_learning_rate), warmup_iters, cosine_cycle_iters)
...
def get_tokenizer(
    ...
    """
    from tokenizer import BPETokenizer, BPEParams

    return BPETokenizer(BPEParams(vocab=vocab, merges=merges), special_tokens=special_tokens or [])
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    t = TokenizerTrainer(vocab_size, special_tokens, debug=True).train_on_file(input_path)
